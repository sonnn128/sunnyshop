...
import com.sonnguyen.trendwearshop.service.ProductService;
...
public class ProductController {
    ...
    private final com.sonnguyen.trendwearshop.service.ExcelService excelService;

    private final com.fasterxml.jackson.databind.ObjectMapper objectMapper;

    public ProductController(ProductService productService, com.sonnguyen.trendwearshop.service.ExcelService excelService, com.fasterxml.jackson.databind.ObjectMapper objectMapper) {
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<com.sonnguyen.trendwearshop.payload.response.ApiResponse<?>> deleteProduct(@PathVariable Long id) {
        if (productService.deleteProduct(id)) {
            return ResponseEntity.ok(
                    com.sonnguyen.trendwearshop.payload.response.ApiResponse.builder()
                            .success(true)
    ...
    @PostMapping("/bulk")
    public ResponseEntity<String> bulkCreate(@RequestBody List<com.sonnguyen.trendwearshop.payload.request.BulkProductRequest> products) {
        try {
    ...
    public ResponseEntity<String> uploadFile(@RequestParam("file") org.springframework.web.multipart.MultipartFile file) {
        if (com.sonnguyen.trendwearshop.utils.ExcelHelper.hasExcelFormat(file)) {
            try {
    ...
    public ResponseEntity<List<com.sonnguyen.trendwearshop.payload.response.ProductVariantResponse>> getProductVariants(@PathVariable Long id) {
        return ResponseEntity.ok(productService.getProductVariants(id));
    }

    @PutMapping("/{id}/variants")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<List<com.sonnguyen.trendwearshop.payload.response.ProductVariantResponse>> updateProductVariants(
            @PathVariable Long id,
            @RequestBody List<com.sonnguyen.trendwearshop.payload.response.ProductVariantResponse> variantsRequest) {
        return ResponseEntity.ok(productService.updateProductVariants(id, variantsRequest));
    }
}
