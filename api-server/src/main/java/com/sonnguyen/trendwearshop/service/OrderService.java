...
import com.sonnguyen.trendwearshop.payload.request.OrderRequest;
import com.sonnguyen.trendwearshop.payload.response.OrderResponse;
...
public class OrderService {
        ...
        try {
            ...
            for (CartItemRequest item : request.getCartItems()) {
                ...
                if (variants != null && !variants.isEmpty()) {
                    matchedVariant = productVariantRepository.findByProductAndSizeAndColor(product, size, color)
                            .orElseThrow(() -> new CommonException("Product variant not found for " + product.getName() + " (" + size + "/" + color + ")", HttpStatus.BAD_REQUEST));
                    
                    if (matchedVariant.getQuantity() < item.getQuantity()) {
                        throw new CommonException("Insufficient product variant quantity for product: " + product.getName() + " (" + size + "/" + color + ")", HttpStatus.BAD_REQUEST);
                    }
                } else {
                ...
                if (matchedVariant != null) {
                    matchedVariant.setQuantity(matchedVariant.getQuantity() - item.getQuantity().intValue());
    ...
    private void restoreInventory(OrderDetail orderDetail) {
        ...
        if (product != null) {
            ...
            String size = orderDetail.getSize() != null ? orderDetail.getSize().trim() : "";
            String color = orderDetail.getColor() != null ? orderDetail.getColor().trim() : "";
            ...
            if (variants != null && !variants.isEmpty()) {
                ...
                if (variantOpt.isPresent()) {
                    ...
                    variant.setQuantity(variant.getQuantity() + orderDetail.getQuantity().intValue());
